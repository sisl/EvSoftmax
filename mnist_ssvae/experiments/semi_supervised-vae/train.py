...
from lvmhelpers.sum_and_sample import SumAndSampleWrapper, SumAndSample
from lvmhelpers.sfe import SFEWrapper, SFEDeterministicWrapper, ScoreFunctionEstimator
from lvmhelpers.nvil import NVILWrapper, NVIL
from lvmhelpers.gumbel import GumbelSoftmaxWrapper, Gumbel
from lvmhelpers.sparsesoftmax import SparseSoftmaxLoss
...
class SSVAE(pl.LightningModule):
    def __init__(
        ...
        gaussian_vae = MNISTVAE(inference_net, generative_net)

        classifier_net = Classifier(
            slen=self.hparams.slen, n_classes=self.hparams.n_classes
        )
        ...
        if self.hparams.mode == "sfe":
            classifier_net = SFEWrapper(
                classifier_net, baseline_type=self.hparams.baseline_type
            )
        ...
        self.lvm_method = lvm_method(
            classifier_net, gaussian_vae, loss_fun, encoder_entropy_coeff=1.0
        )
    ...
    def training_step(self, batch, batch_nb):
        ...
        if not self.hparams.labeled_only:
            unsupervised_output = self(
                unlabeled_batch_image, unlabeled_batch_image, unlabeled_batch_labels
            )
            unsupervised_loss = unsupervised_output["loss"]

            loss = supervised_loss + unsupervised_loss * (
                self.num_unlabeled / self.num_labeled
            )
        else:
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if "support" in validation_result["log"].keys():
            result.log(
                "val_support", validation_result["log"]["support"], prog_bar=True
            )
        return result
    ...
    def test_step(self, batch, batch_nb):
        ...
        if "support" in test_result["log"].keys():
            result.log("test_support", test_result["log"]["support"], prog_bar=True)
        return result
    ...
    def train_dataloader(self):
        ...
        if not self.hparams.labeled_only:
            concat_dataset = CycleConcatDataset(train_labeled, train_unlabeled)
...
def get_reconstruction_loss(x_reconstructed, x):
    ...
    bce_loss = -x * torch.log(x_reconstructed + 1e-8) - (1 - x) * torch.log(
        1 - x_reconstructed + 1e-8
    )
...
def get_elbo_loss(image, vae_output):
    ...
    kl_divergence_loss = get_kl_divergence_loss(latent_means, 2 * torch.log(latent_std))
    return reconstruction_loss + kl_divergence_loss


def get_unsupervised_loss(
    _classifier_input, discrete_latent_z, _vae_input, vae_output, true_labels
):
...
def get_supervised_loss(vae, classifier, labeled_image, true_labels, normalizer):
